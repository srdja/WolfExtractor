...
static int OPLOpenTable( void )
{
	...
	if( (TL_TABLE = (INT32*)MM_MALLOC(TL_MAX*2*sizeof(INT32))) == NULL)
		return 0;
	if( (SIN_TABLE = (INT32**)MM_MALLOC(SIN_ENT*4 *sizeof(INT32 *))) == NULL)
	{
		MM_FREE(TL_TABLE);
		return 0;
	}
	if( (AMS_TABLE = (INT32*)MM_MALLOC(AMS_ENT*2 *sizeof(INT32))) == NULL)
	{
		MM_FREE(TL_TABLE);
		MM_FREE(SIN_TABLE);
		return 0;
	}
	if( (VIB_TABLE = (INT32*)MM_MALLOC(VIB_ENT*2 *sizeof(INT32))) == NULL)
	{
		MM_FREE(TL_TABLE);
		MM_FREE(SIN_TABLE);
		MM_FREE(AMS_TABLE);
